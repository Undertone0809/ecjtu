...
class User:
    ...
    jwxt_login = (
        "https://jwxt.ecjtu.edu.cn/stuMag/Login_dcpLogin.action"  # 教务系统登录页
    )
    ...
    def login(
        ...
        response = session.post(
            User.ecjtu_login, data=login_data, headers=headers, allow_redirects=False
        )
        ...
        # 获取教务JSESSIONID
        session.get(
            User.jwxt_login, headers=headers, allow_redirects=False, verify=False
        )
        # 获取教务跳转登录链接
    ...
    def get_class_table_tofile(self) -> str:
        ...
        for date in date_range:
            classes = self.session.post(
                self.Get_classes, data={"date": date.strftime("%Y-%m-%d")}
            )
            class_info_initial["单双周"] = (
                "单周" if classes.json()["dsWeek"] == 1 else "双周"
            )
        ...
        for week in df["周次"].unique():
            for i in range(1, 8):
                ...
                if df_weekday.empty:
                    ...
                    df[(df["星期"] == i) & (df["周次"] == week)].to_excel(
                        writer, sheet_name=f"第{week}周_星期{i}", index=False
                    )
            writer._save()
    ...
    def get_grade_from_semester(self, semester: str) -> dict:
        ...
        grade_from_semester_initial = {"course_term": semester, "grade_info": []}
    ...
    def get_grade_from_semester_zh(self, semester: str) -> dict:
        ...
        pattern = re.compile(r"\b{}\b".format(tsemester))
        ul_tags = soup.find_all("ul", class_=pattern)
        grade_from_semester_initial = {"课程学期": semester, "课程": []}
        for ul_tag in ul_tags:
            li_list = ul_tag.find_all("li")
            li_values = [li.text for li in li_list]
            grade_from_semester = {
